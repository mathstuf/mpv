                                 ...
                                 uint64_t start)
{
    ...
    int64_t join_diff = start - tl_info->last_end_time;
    if (tl_info->num_parts == 0
        || FFABS(join_diff) > mpctx->opts->chapter_merge_threshold * 1e6
        || source != (*tl_info->timeline)[tl_info->num_parts - 1].source)
    {
        struct timeline_part new = {
            .start = tl_info->start_time / 1e9,
            .source_start = start / 1e9,
            .source = source,
        };
        MP_TARRAY_APPEND(NULL, *tl_info->timeline, tl_info->num_parts, new);
    } else if (tl_info->num_parts > 0 && join_diff) {
        ...
        MP_VERBOSE(mpctx, "Merging timeline part %d with offset %g ms.\n",
                   tl_info->num_parts, join_diff / 1e6);
        tl_info->start_time += join_diff;
        return join_diff;
    }
                                ...
                                int current_source)
{
    ...
    struct demuxer *source = tl_info->sources[current_source];
    ...
    for (int i = 0; i < m->num_ordered_chapters; i++) {
        ...
        for (int j = 0; j < tl_info->num_sources; j++) {
            struct demuxer *linked_source = tl_info->sources[j];
            ...
            if (current_source == j || !linked_m->num_ordered_chapters) {
                ...
                if (source_full_length <= c->start) {
                    tl_info->missing_time += chapter_length;
                    chapter_length = 0;
                    goto found;
                }
                ...
                if (source_length < chapter_length) {
                    tl_info->missing_time += chapter_length - source_length;
                    chapter_length = source_length;
                }

                join_diff = add_timeline_part(mpctx, tl_info,
                ...
                build_timeline_loop(mpctx, chapters, tl_info, &new_info, j);
                // Already handled by the loop call.
                chapter_length = 0;
            }
            tl_info->last_end_time = c->end;
            goto found;
        }
        ...
        chapter_length = 0;
    found:;
        tl_info->start_time += chapter_length;
        ...
        if (info->limit && local_starttime >= info->limit) {
            ...
            tl_info->start_time -= local_starttime - info->limit;
            break;
        }
    }
    ...
    if (local_starttime < info->limit)
        tl_info->missing_time += info->limit - local_starttime;
}
...
void build_ordered_chapter_timeline(struct MPContext *mpctx)
{
    ...
    build_timeline_loop(mpctx, chapters, &tl_info, &info, 0);
    ...
    if (!tl_info.num_parts) {
        ...
        MP_TARRAY_APPEND(NULL, timeline, tl_info.num_parts, new);
    }
     ...
     * revisited. */
    if (tl_info.missing_time >= 1e6) {
        MP_ERR(mpctx, "There are %.3f seconds missing from the timeline!\n",
               tl_info.missing_time / 1e9);
    }
    talloc_free(mpctx->sources);
    mpctx->sources = sources;
    mpctx->num_sources = num_sources;
    mpctx->timeline = timeline;
    mpctx->num_timeline_parts = tl_info.num_parts - 1;
